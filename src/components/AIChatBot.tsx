...
export default function AIChatBot({ includeData = true }: AIChatBotProps) {
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      setMessages(prev => [
        ...prev,
        { role: 'assistant', content: `❌ Error: ${e.message || 'Something went wrong. Please try again.'}` },
      ]);
    } finally {
      setIsLoading(false);
    }
    ...
    <>
      {!isOpen && (
        <button
          ...
          className="fixed bottom-24 right-6 z-50 h-14 w-14 rounded-full bg-primary text-primary-foreground shadow-lg flex items-center justify-center hover:scale-105 transition-transform"
        >
